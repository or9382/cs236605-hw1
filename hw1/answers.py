...
the bigger the margin between answers needs to be, the bigger the weights can grow in order to meet that expectation without changing the actual relations between the scores.  
...
because they both control the same trade-off (weights' size to accuracy) delta can be chosen arbitrarily as long as lambda is tuned correctly with said arbitrary value.
"""
...
In this notion, the KNN with k=1 and the templates being the points it remembers, should be equivilent to the linear classifier.
"""
...
2) Based on the graph of the Training and validation sets' accuracy we think that the model is **slightly overfitted to the training set**.  
...
Based on the above residual plot we can say that the training examples predictions were closer to their actual score compared to the test examples.  
This could be seen by the fact that the training points are more compact around the y-hat axis.  
The plot gotten by the top-5 features looks much nicer than the final plot as its points are seen to be within about 1 unit of error.
"""
...
For any combination we used kfold with k=3 to fit the data and then test it.
